...
import { Loader2, Pickaxe, RotateCcw } from "lucide-react";
...
export default function MiningLoadoutPage() {
  ...
  function setLaser(ti: number, name: string) {
    setLoadout((prev) => {
      const next = prev.map((t) => ({ laser: t.laser, modules: [...t.modules] }));
      next[ti].laser = name;
      const l = L[name];
  ...
  function laserOptions(size: number): DropdownOption[] {
    const ls = (data?.lasers ?? [])
      .filter((l) => l.size === size)
      .sort((a, b) => (b.maxPower ?? 0) - (a.maxPower ?? 0));
    return [
      { value: "", label: NONE },
      ...ls.map((l) => ({ value: l.name, label: `${l.name}${l.price ? ` · ${fmtAUEC(l.price)}` : ""}` })),
    ];
  }
  function moduleOptions(): DropdownOption[] {
    ...
    return [
      { value: "", label: NONE },
      ...ms.map((m) => ({ value: m.name, label: `${m.name} · ${m.type === "Active" ? "actif" : "passif"}` })),
    ];
  }
  ...
  // pièces équipées (uniques) pour « où acheter »
  const parts = useMemo(() => {
      ...
      <div className="mb-6 flex items-end justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 text-2xl font-bold text-white">
            ...
            {t("miningLoadout.title")}
          </h1>
          <p className="mt-1 flex items-center gap-2 text-sm text-white/50">
            {t("miningLoadout.subtitle")}
            <span className="rounded-full border border-white/10 px-2 py-0.5 font-mono text-[10px] tracking-wider text-white/40">
              UEX
            </span>
          </p>
        </div>
                        ...
                        <div className="flex flex-col gap-2.5">
                          ...
                          {Array.from({ length: slots }).map((_, si) => (
                            <div key={si} className="flex items-center gap-3">
                              ...
                              <div className="min-w-0 flex-1">
                                <Dropdown
                                  value={turret.modules[si] ?? ""}
                                  onChange={(v) => setModule(ti, si, v)}
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                    <div className="mb-3 flex items-center gap-2">
                      ...
                      <span className="ml-auto font-mono text-xs text-amber-300">
                        {G[gadget]?.price ? fmtAUEC(G[gadget].price) : ""}
                      </span>
                    </div>
                    <div className="flex items-center gap-3">
                      <label className="w-16 shrink-0 font-mono text-[9px] uppercase tracking-wider text-white/40">
                        {t("miningLoadout.gadget")}
                      </label>
                      <div className="min-w-0 flex-1">
              ...
              {cfg.fixed ? (
                ...
                <>
                  <div className="font-mono text-[9px] uppercase tracking-[0.14em] text-white/40">
                    {t("miningLoadout.power")}
                  </div>
                  <div className="mt-1 font-mono text-2xl font-medium text-white">
                    {anyLaser ? `${fmtPow(stats.minP)}–${fmtPow(stats.maxP)}` : "—"}{" "}
                  ...
                  <div className="mb-2 mt-4 font-mono text-[9px] uppercase tracking-[0.14em] text-white/40">
                    {t("miningLoadout.modifiers")}
                  </div>
                      ...
                      <div key={k} className="flex items-center gap-2 border-b border-white/[0.06] py-1.5">
                        ...
                        <span className="relative h-1 w-16 overflow-hidden rounded bg-black/40">
                          <i
                            ...
                            style={{
                              width: `${w}%`,
                              left: v >= 0 ? "50%" : `${50 - w}%`,
                              background: neutral ? "var(--accent)" : good ? "#46e6a0" : "#ff6b6b",
                        ...
                        <span
                          className={[
                            ...
                            neutral ? "text-white/40" : good ? "text-emerald-400" : "text-red-400",
                          ].join(" ")}
                        >
                      ...
                      <div className="flex flex-col gap-1.5">
                        {parts.map((p) => {
                          ...
                          return (
                            <div
                              key={p.name}
                              className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/30 px-2.5 py-1.5"
                            >
                  ...
                  {!anyLaser && (
                    <p className="mt-3 text-[12px] leading-relaxed text-white/40">{t("miningLoadout.pickLaser")}</p>
                  )}
                </>
              )}
            </aside>
