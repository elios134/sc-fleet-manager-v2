  ...
  return (
    <div className="flex h-full flex-col gap-3 p-6">
      <header className="flex shrink-0 items-baseline gap-3">
        ...
        <h1 className="text-2xl font-bold text-white">CARTE GALACTIQUE</h1>
        {!loading && !error && (
          <span className="text-[11px] tabular-nums text-white/40">
            {bodies.length} corps — {systems.join(" / ")}
          </span>
        )}
      </header>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-dashed border-white/15 bg-white/5 p-10 text-center text-white/70">
          Carte vide — synchronisez la carte galactique depuis Settings.
        </div>
      ) : (
